...
class Scene private(val objects: List[Shape], val lights: List[Light]) {
  ...
  def traceImage(width: Int, height: Int) {
    ...
    val system = ActorSystem("ActorSystem")
      // Create a pool of actors with RoundRobin routing algorithm- per pixel hence the pool size = width
    val worker = system.actorOf(Props[Coordinator.CoordinatorActor].withRouter(RoundRobinPool(width)),"worker")

     
    for (y <- 0 until height) { //for each round of y <- 0 until height coordinator needs to spin off a worker
      for (x <- 0 until width) { // for each 
        ...
        worker ! MessageFromScene(x, y, colour)
      }
